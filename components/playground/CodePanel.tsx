...
export default function CodePanel({ code, activeLine, language = "javascript" }: CodePanelProps) {
  // Split code into lines and handle the "1: " prefix or auto-generate line numbers
  const lines = code.split("\n").map((line, idx) => {
    const match = line.match(/^(\d+):\s*(.*)/);
    if (match) {
      return { number: parseInt(match[1]), content: match[2] };
    }
    // If no prefix, use 1-based index as line number
    return { number: idx + 1, content: line };
  });
      ...
      <div className="flex-1 overflow-auto p-4 custom-slim-scrollbar font-mono text-[11px] leading-relaxed relative">
        <div className="absolute left-0 top-0 bottom-0 w-8 bg-[#0D152B]/50 border-r border-white/5 flex flex-col items-center pt-4 pointer-events-none">
          {lines.map((line, i) => (
            <div
              key={i}
              className={`h-[1.5em] flex items-center justify-center transition-colors ${line.number === activeLine ? 'text-white font-black' : 'text-slate-600'}`}
            >
              {line.number || ""}
            </div>
          ))}
        ...
        <div className="pl-8">
          {lines.map((line, i) => (
            <div
              key={i}
              className={`h-[1.5em] px-2 relative transition-all duration-200 group ${line.number === activeLine ? 'text-white font-black' : 'text-[#DBE7FF]/70 hover:text-white hover:bg-white/5'}`}
            >
              {line.number === activeLine && (
                <motion.div
                  layoutId="active-line-bg"
                  className="absolute inset-0 bg-[#0052FF] -z-10 border-l-4 border-white"
                  initial={false}
