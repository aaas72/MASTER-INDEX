...
export default function VersusHubPage() {
  ...
  const options = algorithmEntries.map(([id, algo]) => ({ id, name: algo.metadata.title }));
        ...
        {showResults ? (
          <div className="flex-1 flex flex-col animate-in fade-in zoom-in-95 duration-300">
            <div className="bg-surface-container border-2 border-primary shadow-lg overflow-hidden mb-6">
              ...
              {isAnalysisMode && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="border-b-2 border-primary/10"><div className="grid grid-cols-2 divide-x-2 divide-primary/10 bg-white"><div className="p-4 flex flex-col gap-2"><div className="flex items-center justify-between"><span className="font-mono text-[9px] text-primary font-bold uppercase tracking-widest">{typedAlgorithmsData[leftAlgorithmId]?.metadata.title}</span><span className="font-mono text-[9px] bg-primary/10 text-primary px-2 py-0.5">{leftMsg}</span></div><div className="h-[200px] bg-surface-container-low/20 flex items-center justify-center overflow-hidden border border-primary/5">{renderVisualizer('left', 180)}</div></div><div className="p-4 flex flex-col gap-2"><div className="flex items-center justify-between"><span className="font-mono text-[9px] text-primary font-bold uppercase tracking-widest">{typedAlgorithmsData[rightAlgorithmId]?.metadata.title}</span><span className="font-mono text-[9px] bg-primary/10 text-primary px-2 py-0.5">{rightMsg}</span></div><div className="h-[200px] bg-surface-container-low/20 flex items-center justify-center overflow-hidden border border-primary/5">{renderVisualizer('right', 180)}</div></div></div></motion.div>)}
            </div>
            <div className="grid grid-cols-2 border-2 border-outline-variant/30 bg-surface-container-lowest overflow-hidden shrink-0">
              <VersusCompetitorLeft algorithm={typedAlgorithmsData[leftAlgorithmId]} isWinner={true} visualizer={renderVisualizer('left', 300)} summaryPoints={[`Domain: ${typedAlgorithmsData[leftAlgorithmId]?.metadata.category}`, `Time: ${typedAlgorithmsData[leftAlgorithmId]?.complexity.time.average}`, `Space: ${typedAlgorithmsData[leftAlgorithmId]?.complexity.space.average}`]} />
              <VersusCompetitorRight algorithm={typedAlgorithmsData[rightAlgorithmId]} isWinner={true} visualizer={renderVisualizer('right', 300)} summaryPoints={[`Domain: ${typedAlgorithmsData[rightAlgorithmId]?.metadata.category}`, `Time: ${typedAlgorithmsData[rightAlgorithmId]?.complexity.time.average}`, `Space: ${typedAlgorithmsData[rightAlgorithmId]?.complexity.space.average}`]} />
            </div>
            <div className="mt-8"><VersusCenterRadar leftName={getAbbr(typedAlgorithmsData[leftAlgorithmId]?.metadata.title || '')} rightName={getAbbr(typedAlgorithmsData[rightAlgorithmId]?.metadata.title || '')} speedDeltaPercent={speedDeltaPercent} /></div>
          </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center bg-surface-container-low/20 backdrop-blur-sm min-h-[400px]">
            {isVisualSim && (<div className="w-full h-full grid grid-cols-2 gap-12 p-8"><div className="flex flex-col gap-4"><div className="flex items-center justify-between"><p className="font-mono text-[10px] text-primary uppercase tracking-[0.4em]">{typedAlgorithmsData[leftAlgorithmId]?.metadata.title}</p></div><div className="flex-1 border border-outline-variant/20 bg-white h-[300px] flex items-center justify-center">{renderVisualizer('left', 280)}</div></div><div className="flex flex-col gap-4"><div className="flex items-center justify-between"><p className="font-mono text-[10px] text-primary uppercase tracking-[0.4em]">{typedAlgorithmsData[rightAlgorithmId]?.metadata.title}</p></div><div className="flex-1 border border-outline-variant/20 bg-white h-[300px] flex items-center justify-center">{renderVisualizer('right', 280)}</div></div></div>)}
          </div>
        ) : (
