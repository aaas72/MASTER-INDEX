...
export function simulateBSTInsertion(): LogicStep[] {
  ...
  const ns: SimulationNode[] = [];
  const es: SimulationEdge[] = [];
  ...
  for (const val of values) {
    ...
    steps.push({ description_en: `Insert ${val} into BST. ${ns.length === 1 ? 'Becomes root.' : `Placed as ${val < 50 ? 'left' : 'right'} child.`}`, nodes_state: cloneState(ns), edges_state: cloneState(es), active_line: 2 });
    ns.find(n => n.id === String(val))!.state = "default";
  }
  steps.push({ description_en: "BST construction complete. All 7 nodes inserted.", nodes_state: cloneState(ns), edges_state: cloneState(es), active_line: -1 });
  return steps;
}
...
export function simulateInorderTraversal(): LogicStep[] {
  ...
  steps.push({ description_en: "Start inorder traversal (Left → Root → Right).", nodes_state: cloneState(ns), edges_state: cloneState(es), active_line: 1 });
  ...
  function inorder(nodeId: string) {
    ...
    steps.push({ description_en: `Visit node ${nodeId}. Output so far: [${result.join(', ')}].`, nodes_state: cloneState(ns), edges_state: cloneState(es), active_line: 4 });
    node.state = "visited";
  ...
  inorder("50");
  steps.push({ description_en: `Inorder traversal complete: [${result.join(', ')}].`, nodes_state: cloneState(ns), edges_state: cloneState(es), active_line: -1 });
  return steps;
}
...
export function simulateMaxHeapify(): LogicStep[] {
  ...
  const steps: LogicStep[] = [];
  const buildNodes = (a: number[]): SimulationNode[] => [
  ...
  const baseEdges: SimulationEdge[] = [
  ...
  steps.push({ description_en: `Initial array: [${arr.join(', ')}]. Build Max-Heap.`, array_state: [...arr], nodes_state: buildNodes(arr), edges_state: cloneState(baseEdges), active_line: 1 });
  ...
  function heapify(a: number[], n: number, i: number) {
    ...
    steps.push({ description_en: `Heapify at index ${i} (val=${a[i]}). Children: L=${l < n ? a[l] : 'none'}, R=${r < n ? a[r] : 'none'}. Largest=${a[largest]}.`, array_state: [...a], nodes_state: buildNodes(a), edges_state: cloneState(baseEdges), highlight_indices: [i, largest], active_line: 3 });
    if (largest !== i) {
      [a[i], a[largest]] = [a[largest], a[i]];
      steps.push({ description_en: `Swap ${a[largest]} ↔ ${a[i]} (indices ${i} and ${largest}).`, array_state: [...a], nodes_state: buildNodes(a), edges_state: cloneState(baseEdges), highlight_indices: [i, largest], active_line: 5 });
      heapify(a, n, largest);
    }
  }
  for (let i = Math.floor(arr.length / 2) - 1; i >= 0; i--) { heapify(arr, arr.length, i); }
  steps.push({ description_en: `Max-Heap built: [${arr.join(', ')}]. Root = ${arr[0]}.`, array_state: [...arr], nodes_state: buildNodes(arr), edges_state: cloneState(baseEdges), active_line: -1 });
  return steps;
}
